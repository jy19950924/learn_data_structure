...
void Graph::_display(vector<vector<string>>& matrix, map<Node*, int>& collects) {
...
Node* Graph::_getUnselectedMinNode(set<Node*>& passingNode, map<Node*, int>& cost) {
  // 获取未选择的最小cost值的点（临时版本）
  Node* res = nullptr;
  map<int, Node*> sortCost;
  std::transform(cost.begin(), cost.end(), inserter(sortCost, sortCost.begin()), [](const auto& iter) { return std::pair<int, Node*>{iter.second, iter.first}; });
...
int testDijkstra() {
  ...
  vector<vector<string>> connects = { {"begin", "A", "1"}, {"begin", "B", "2"}, {"B", "A", "3"}, {"A", "end", "1"}, {"B", "end", "5"} };
